...
export async function* respond({
  ...
  // const rawUserInput = "I need help solving x² - 5x + 6 = 0. I think I have to factor it, but I don’t know how.";
  const detectedPhase = await detectPhaseWithLLM(message);
  function wrapUserMessageByPhase(userInput: string, phase: "before" | "during" | "after"): string {
    const instructionByPhase = {
      before: "Please ask a clarifying question. Do not solve or explain yet.",
      during: "Guide the learner step by step through reasoning. Do not give the final answer.",
      after: "Summarize what they’ve learned and ask for reflection or a next step."
    };
  
    return `[Phase: ${phase.toUpperCase()}] The learner said: "${userInput}" ${instructionByPhase[phase]}`;
  }
// 2. Wrap the input accordingly
console.log("detectedPhase", detectedPhase);
const wrappedUserMessage = wrapUserMessageByPhase(message, detectedPhase);

  console.log("wrappedUserMessage",wrappedUserMessage);
  // Generate response
